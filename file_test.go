...
func TestFileReadExe(t *testing.T) {
	...
	// DOS Header
	assert.Equal(t, p.DOSHeader.Size(), int64(64))
	assert.Equal(t, p.DOSHeader.Data.Lfanew, uint32(64+128))

	// DOS Stub
	assert.Equal(t, p.DOSStub.Size(), int64(128))
	...
	assert.Equal(t, p.COFFHeader.Data.SizeOfOptionalHeader, uint16(224))
}
